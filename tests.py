...
class UserModelCase(unittest.TestCase):
    ...
    def test_follow(self):
        ...
        #Prüfung der follow()-Methode
        u1.follow(u2)
        db.session.commit()
        self.assertTrue(u1.is_following(u2)) #Folgt u1 u2?
        self.assertEqual(u1.followed.count(), 1)
        self.assertEqual(u1.followed.first().username, 'susan')
        self.assertEqual(u2.followers.count(), 1) #Hat u2 genau einen Follower?
        self.assertEqual(u2.followers.first().username, 'john') #Stimmt der Username für u2? 

        #Prüfung der unfollow()-Methode
        u1.unfollow(u2)
        db.session.commit()
